...
var supabase = (0, import_supabase_js.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_PUBLIC_ANON_KEY);

// src/db/fetch-case-studies.ts
var fetchCaseStudies = () => __async(void 0, null, function* () {
  return supabase.from("case_study").select("*, author (*)");
});

// src/routes/case-studies.ts
var express = require("express");
var caseStudyRouter = express.Router();
caseStudyRouter.get("/case-studies", (request, response) => __async(void 0, null, function* () {
  const { data: caseStudies, error } = yield fetchCaseStudies();
  if (error) {
    console.error(error);
  ...
  if (!caseStudies) {
    response.status(statusCodes.NOT_FOUND).json({ message: "Nothing here." });
    return;
  }
  if (caseStudies.length === 0) {
    response.status(statusCodes.NO_CONTENT).json({ message: "No case studies have been added ...yet." });
    return;
  }
...
// src/middleware/json-from-body.ts
var express2 = require("express");
var jsonFromBody = (request, response, next) => express2.json()(request, response, next);
...
var CURRENT_HOST = "http://localhost:431";
...
app.listen(config_default.port, () => {
  console.log("server ready on port:", config_default.port);
});
/*
object-assign
(c) Sindre Sorhus
